# Meridius Animated Logo Mark — Native macOS SwiftUI Guide

This document contains full instructions and the complete, drop-in Swift code to integrate the exact **Meridius Animated 3D Dotted Logo Mark** from the website into the native Meridius macOS application.

---

## 1. Overview & Architecture

* **Framework**: Native SwiftUI (`Canvas` + `TimelineView`)
* **Compatibility**: macOS 12.0+ (Universal / Apple Silicon & Intel)
* **Dependencies**: **None** (Zero third-party packages, zero Lottie JSONs, zero video assets).
* **Performance**: Hardware-accelerated 60fps / 120fps ProMotion on Retina displays (<0.2ms CPU time per frame).
* **Theme Support**: Real-time automatic Light & Dark Mode handling with 3D isometric cube face lighting.

---

## 2. Dynamic Dark / Light Mode Handling

The component uses SwiftUI’s `@Environment(\.colorScheme)` to observe macOS appearance changes dynamically.

### Lighting & Shading Rules:

| Element | **Dark Mode** (`colorScheme == .dark`) | **Light Mode** (`colorScheme == .light`) |
| :--- | :--- | :--- |
| **"M" Shape (Expanded Wave)** | Solid Cream (`#f5f5ee` / 100% opacity) | Solid Charcoal (`#2b2b2a` / 100% opacity) |
| **Top Face Dots (Background)** | Cream at **90%** opacity *(Illuminated from above)* | Charcoal at **35%** opacity *(Light tone)* |
| **Left Face Dots (Background)** | Cream at **60%** opacity *(Mid-tone)* | Charcoal at **65%** opacity *(Mid-tone)* |
| **Right Face Dots (Background)** | Cream at **35%** opacity *(In shadow)* | Charcoal at **92%** opacity *(Deep shadow)* |

---

## 3. Complete Source Code: `AnimatedLogoMarkView.swift`

Add this file to your Xcode project under your UI or Components folder:
...
## 4. How to Use in SwiftUI Views

### A. Onboarding / Welcome Hero Screen
```swift
AnimatedLogoMarkView(size: 260)
    .padding(.bottom, 24)
```

### B. Settings or About View
```swift
VStack(spacing: 12) {
    AnimatedLogoMarkView(size: 96)
    Text("Meridius for Mac")
...
### C. Responsive Auto-Fitting Container
```swift
AnimatedLogoMarkView()
    .frame(maxWidth: 320, maxHeight: 320)
```

### D. Manual Dark/Light Mode Overrides (if needed)
```swift
// Explicitly force dark mode regardless of macOS system settings
AnimatedLogoMarkView(size: 180)
